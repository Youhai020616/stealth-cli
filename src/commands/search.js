...
import { daemonExtract } from "../client.js";
...
export function registerSearch(program) {
  program
    ...
    .option(
      "--warmup",
      "Visit a random site before searching (helps bypass detection)",
    )
      ...
      try {
        ...
        if (opts.format === "snapshot") {
          ...
          if (!handle.isDaemon) {
            ...
            // Daemon mode: use /extract route for server-side parsing
            const res = await daemonExtract(engine, parseInt(opts.num, 10), {
              alsoAsk: isGoogle && !!opts.alsoAsk,
            });

            if (res?.ok && res.results?.length > 0) {
              results = res.results;
              if (res.peopleAlsoAsk?.length > 0) {
                alsoAsk = res.peopleAlsoAsk;
              }
            } else {
